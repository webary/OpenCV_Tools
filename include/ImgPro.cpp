...
IplImage *loadImg(cchar* tmpimgName, int ask, int cls, int color, int show)
{
    ...
    do {
        ...
        if (!isFileExists(imgName, 0)) {                    //打开失败,没有读取权限
            strcpy(tempName, imgName);
            if (strstr(tempName, ".jpg") == 0) {            //文件名没有以.jpg结尾
                strcat(tempName, ".jpg");
                if (!isFileExists(tempName, 0)) {           //打开失败,没有读取权限
                    strcpy(tempName, imgName);
        ...
        if (isFileExists(imgName, 0) && (strstr(imgName, ".jpg") || strstr(imgName, ".bmp") || strstr(imgName, ".png"))) {                      //图片可以读取
...
{
    IplImage *object = cvCreateImage(cvGetSize(obj_color), 8, 1); //单通道
    IplImage *image  = cvCreateImage(cvGetSize(img_color), 8, 1); //单通道
    IplImage* object_surf = cvCreateImage(cvGetSize(object), 8, 3); //3通道
    ...
    //如果只是匹配两幅相似大小的图，可以通过斜率来过滤掉不相干的角点匹配对
    //double error = 0.13;
...
{
#ifndef __AFXWIN_H__
    IplImage* object = loadImg(objName, 0, 0, 1, showInfo); //读取彩图,保证彩色显示
    IplImage* image = loadImg(imgName, 0, 0, 1, showInfo);  //读取彩图,保证彩色显示
#else
...
class WM
{
    ...
    vector<uchar> data,gray; //用来读取数组的模板, 灰度数组
public:
